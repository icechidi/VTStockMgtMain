...
const FALLBACK_LOCATIONS = ["Main Warehouse", "Store A", "Store B", "Distribution Center", "Office Storage"]
...
export default function NewMovementPage() {
  ...
  const [locationOptions, setLocationOptions] = useState<string[]>(FALLBACK_LOCATIONS)
  ...
  useEffect(() => {
    if (session?.user?.name) {
      setFormData((prev) => ({ ...prev, user_name: session.user!.name as string }))
    }
  }, [session?.user?.name])
  ...
  useEffect(() => {
    fetch("/api/suppliers")
      ...
      .then((data) => {
        const rows: Supplier[] = Array.isArray(data) ? data : (data?.suppliers ?? [])
        setSuppliers(rows)
      })
    ...
    fetch("/api/locations")
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        const rows: LocationRow[] = Array.isArray(data) ? data : (data?.locations ?? [])
        if (rows.length > 0) setLocationOptions(rows.map((l) => l.name))
      })
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-xl font-semibold tracking-tight">Record Stock Movement</h2>
      </div>
